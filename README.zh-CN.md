...
本项目面向室内服务、远程巡检、无人值守看护和机器人教学科研场景，目标是让机器人完成从环境感知、建图定位、路径导航、视觉识别、语义地图构建到自然语言任务执行的闭环。
...
文件统计基于当前仓库快照，忽略隐藏目录，仅统计工程可见内容。仓库约有 **530 个可见文件**，其中 ROS 2 源码与第三方驱动主要集中在 `src/`。

| 路径 | 文件数 | 作用 |
| --- | ---: | --- |
| `src/` | 430 | ROS 2 工作空间源码，包含自研机器人功能包、传感器驱动和第三方 SDK。 |
| `calib_images/` | 50 | 项目自摄校准图片，当前为一组 `model_calib_20260514_210005` 标定样本。 |
...
| `models/` | 4 | RDK X5 BPU 可运行的 YOLOv8s Open Images V7 模型、类别文件、转换日志和模型卡。 |
| `digua_navigation_data/` | 1 | 导航业务数据，当前保存 `named_poses.yaml` 命名点位。 |
| `test_logs/` | 1 | 调试记录与测试日志，当前包含视觉 ICP/TF 静态测试日志。 |
| `Ackermann_Steering_Chassis_Models/` | 1 | 项目自绘阿克曼底盘机械 STEP 模型。 |
| 根目录文件 | 8 | Git 配置、许可证、双语 README、快速开始文档、第三方声明和相机反馈样例图等入口文件。 |
...
├── THIRD_PARTY_NOTICES.md            # 第三方与资产声明
...
- 邮箱：[wxh_13424290329@qq.com](mailto:wxh_13424290329@qq.com)
...
除另有说明外，本项目原创代码采用根目录 [Apache-2.0](./LICENSE) 许可证。

本仓库同时包含官方第三方驱动/SDK、借鉴或改编代码、模型文件和运行数据；这些内容不自动适用根目录 Apache-2.0。具体来源、许可证状态和二次分发注意事项见 [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md)。
